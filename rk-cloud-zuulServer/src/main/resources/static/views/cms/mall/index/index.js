...
function(perfect,switchery,main,index,rkTab,rkAjax,rkAlert,rkUtil) {
	...
	var loadModule=function(){
		rkAjax.postAjax("/rk/admin/core/domain/module/queryList",null,true,
				function(data){
	...
	var loadMenu=function(moduleCode){
		...
		rkAjax.postAjax("/rk/admin/core/domain/adminMenu/queryMenu",{moduleCode:moduleCode},true,
			function(data){
				for(var i=0;i<data.length;i++){
					var tag="";
					if(i==0){
						tag+="<li class='active open' id='menu_"+data[i].code+"'>";
					}else{
						tag+="<li id='menu_"+data[i].code+"'>";
					}
					//区分是否有子节点的菜单
					if(!rkUtil.rkStrUtil.isNull(data[i].url)){
						tag+="<a href='#' url='"+data[i].url+"'>";
					}else{
						tag+="<a href='javascript:void(0)' url=''>";
					}
					if(data[i].fatherCode=="-1"){
