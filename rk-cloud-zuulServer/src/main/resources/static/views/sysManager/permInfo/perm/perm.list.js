require(['jquery',rkjsPath+'rkTable.js',rkjsPath+'rkTags.js',rkjsPath+'rkWin.js',rkjsPath+'rkFormUtil.js',rkjsPath+'rkAlert.sweet.js',rkjsPath+'rkAjax.js'], 
	...
	'use strict'
	var permList=rkTable.init("permList", "/rk/admin/core/user/perm/queryPage",{},"radio","searchBtn");
	var btns=rkTags.btn.addEditDelBtn("tableDiv", true, function(){
		rkWin.win("/views/sysManager/permInfo/perm/perm.detail.html",{method:"add"},"新增权限",
		function(){
			rkTable.reload(permList);
		});
	}, function(){
		var ids=rkTable.getChecked("permList");
		rkWin.win("/views/sysManager/permInfo/perm/perm.detail.html",{method:"edit",ajaxUrl:"/rk/admin/core/user/perm/query",id:ids[0]},"编辑权限",
		function(){
			rkTable.reload(permList);
		});
	}, function(){
		var ids=rkTable.getChecked("permList");
		if(ids.length>0){
			rkAjax.postAjaxNoErr("/rk/admin/core/user/perm/delete",{idArr:ids},function(data){
